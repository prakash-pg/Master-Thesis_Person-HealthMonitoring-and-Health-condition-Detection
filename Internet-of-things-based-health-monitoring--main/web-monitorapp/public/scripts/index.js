...
function epochToJsDate(epochTime){
  ...
  const bodytempElement = document.getElementById("bodytemperature");
  const heartrateElement = document.getElementById("heartrate");
  const bloodoxygenElement = document.getElementById("bloodoxygen");
  const tempElement = document.getElementById("roomtemperature");
  const humElement = document.getElementById("roomhumidity");
    ...
    if (user) {
      ...
      chartRef.on('value', snapshot =>{
        ...
        dbRef.orderByKey().limitToLast(chartRange).on('child_added', snapshot =>{
          ...
          plotValues(chartBT, timestamp, bodytemperature);
          plotValues(chartHR, timestamp, heartrate);
        });
      });
      ...
      dbRef.orderByKey().limitToLast(1).on('child_added', snapshot =>{
        ...
        var bodytemperature = jsonData.bodytemperature;
        var heartrate = jsonData.heartrate;
        var bloodoxygen = jsonData.bloodoxygen;
        var roomtemperature = jsonData.roomtemperature;
        var roomhumidity = jsonData.roomhumidity;
        var airquality = jsonData.airquality;
        var timestamp = jsonData.timestamp;
        // Update DOM elements
        bodytempElement.innerHTML = bodytemperature;
        heartrateElement.innerHTML = heartrate;
        bloodoxygenElement.innerHTML = bloodoxygen;
        tempElement.innerHTML = roomtemperature;
        humElement.innerHTML = roomhumidity;
        aqElement.innerHTML = airquality;
      ...
      dbRef.orderByKey().limitToLast(1).on('child_added', snapshot =>{
        ...
        var timestamp = jsonData.timestamp;
        // Update DOM elements
        var gaugeBT = createBodyTemperatureGauge();
        var gaugeHR = createHeartRateGauge();
        gaugeBT.draw();
        gaugeHR.draw();
        gaugeBT.value = bodytemperature;
      ...
      function createTable(){
        ...
        dbRef.orderByKey().limitToLast(100).on('child_added', function(snapshot) {
          if (snapshot.exists()) {
            ...
            content += '<td>' + bodytemperature + '</td>';
            content += '<td>' + heartrate + '</td>';
            content += '<td>' + bloodoxygen + '</td>';
            content += '<td>' + roomtemperature + '</td>';
            content += '<td>' + roomhumidity + '</td>';
            content += '<td>' + airquality + '</td>';
            content += '</tr>';
          ...
          if (snapshot.exists()) {
            ...
            reversedList.forEach(element =>{
              if (firstTime){ // ignore first reading (it's already on the table from the previous query)
                ...
                var heartrate = element.heartrate;
                var bloodoxygen = element.bloodoxygen;
                var roomtemperature = element.roomtemperature;
                var roomhumidity = element.roomhumidity;
                var airquality = element.airquality;
